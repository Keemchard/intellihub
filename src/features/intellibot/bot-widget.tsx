  ...
  return (
    <div className="fixed bottom-6 right-6 z-40 flex h-[min(560px,80vh)] w-[min(400px,calc(100vw-3rem))] animate-fadein flex-col overflow-hidden rounded-2xl border border-border bg-card shadow-float">
      <header className="flex items-center gap-2.5 border-b border-border p-4">
        <div className="grad-brand grid h-9 w-9 place-items-center rounded-xl text-white">
          <Icon name="sparkles" size={17} />
        </div>
        <div className="flex-1">
          <div className="flex items-center gap-1.5 text-sm font-bold">
            IntelliBot
            <span className="rounded bg-muted px-1.5 py-0.5 text-[9px] font-bold tracking-wider text-muted-foreground">
              BETA
            </span>
          </div>
          <div className="text-[11px] text-muted-foreground">
            Your analytics guide
          </div>
        </div>
        {turns.length > 0 && (
          <button
      ...
      <div
        className={cn(
          ...
          !turns.length && "grid place-items-center",
        )}
      >
        {turns.length ? (
          <Conversation turns={turns} isPending={isPending} />
        ) : (
          <div className="text-center">
            <div className="grad-brand mx-auto mb-3 grid h-12 w-12 place-items-center rounded-2xl text-white">
              <Icon name="sparkles" size={22} />
            </div>
            <p className="mb-4 text-sm text-muted-foreground">
              Ask me to find analytics, explain a KPI, or walk you through
              access.
            </p>
      ...
      <div className="border-t border-border p-3">
        <Composer onAsk={ask} disabled={isPending} />
      </div>
    </div>
  );
}
