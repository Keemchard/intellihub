    ...
    <Link href={`/product/${p.id}`} className="ring-focus block rounded-2xl">
      <Card className="h-full p-5 transition-all hover:-translate-y-0.5 hover:shadow-float">
        <div className="mb-3 flex items-start justify-between">
          <div className="grid h-12 w-12 place-items-center rounded-2xl" style={{ background: `${p.accent}1a`, color: p.accent }}>
            <Icon name={p.icon} size={22} />
          </div>
          <TrustBadge trust={p.trust} certified={p.certified} />
        </div>
        <div className="mb-1 flex items-center gap-2"><TypePill type={p.type} /></div>
        ...
        <div className="mt-4 flex items-center justify-between border-t border-border pt-3">
          <span className="text-xs font-medium text-muted-foreground">{p.family}</span>
  ...
  return (
    <Link href={`/product/${p.id}`} className="ring-focus block rounded-2xl">
      <Card className="flex items-center gap-4 p-4 transition hover:shadow-float">
        <div className="grid h-12 w-12 shrink-0 place-items-center rounded-2xl" style={{ background: `${p.accent}1a`, color: p.accent }}>
          <Icon name={p.icon} size={22} />
        </div>
        <div className="min-w-0 flex-1">
          <div className="mb-1 flex flex-wrap items-center gap-2">
            <TypePill type={p.type} /><TrustBadge trust={p.trust} certified={p.certified} />
          </div>
        ...
        <Icon name="arrow-right" size={18} className="shrink-0 text-muted-foreground" />
      </Card>
    </Link>
  );
}
