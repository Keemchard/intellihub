...
import type { ProductType, Kpi } from "@/types";
...
export function TrustBadge({ trust, certified }: { trust: "Trusted" | "In Review"; certified?: boolean }) {
  if (trust === "Trusted")
    return <Badge variant="success"><Icon name="badge-check" size={12} />{certified ? "Certified" : "Trusted"}</Badge>;
  return <Badge variant="warning"><Icon name="clock" size={12} />In Review</Badge>;
}
...
export function Stars({ rating, reviews }: { rating: number; reviews?: number }) {
  return (
    <span className="inline-flex items-center gap-1 text-xs font-semibold text-amber-500">
      ...
      {reviews != null && <span className="text-muted-foreground font-normal">({reviews})</span>}
    </span>
  );
}
